...
export class NotificationsDispatchWorker implements OnModuleInit {
  ...
  private async handleDispatch(job: Job<DispatchPayload>): Promise<any> {
    ...
    const rec = await this.prisma.notificationsQueue.findFirst({
    ...
    const contact =
      rec.patient?.contacts.find((c: any) => c.type.toUpperCase() === contactType) ||
    ...
    // 3. Fetch appointment and employee from scheduling database
    let appointment = null;
    let employee = null;
    if (rec.appointmentId) {
      appointment = await this.schedulingPrisma.appointment.findUnique({
    ...
    // 5. Invoke channel provider send call
    ...
    // 6. Update queue log & audit entities
    await this.prisma.notificationsQueue.update({
      where: { id: rec.id },
      data: {
    ...
    if (rec.appointmentId) {
      await this.schedulingPrisma.appointmentNotification.create({
        data: {
          tenantId,
