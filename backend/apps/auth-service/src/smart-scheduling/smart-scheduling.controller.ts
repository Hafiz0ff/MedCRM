...
} from '../../../scheduling-service/src/smart-scheduling/dto/appointment.schemas';
...
export class SmartSchedulingController {
  ...
  @Get('doctors')
  @RequirePermissions('scheduling.availability.read')
  @ApiOperation({ summary: 'List all employees available for scheduling' })
  getDoctors(@CurrentUser() user: AuthenticatedUser) {
    return this.scheduling.getDoctors(user);
  }
  ...
  getServices(@CurrentUser() user: AuthenticatedUser) {
    return this.scheduling.getServices(user);
  }
}
