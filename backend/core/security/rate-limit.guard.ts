...
export class RateLimitGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const isHeavy = this.reflector.getAllAndOverride<boolean>(HEAVY_ENDPOINT_KEY, [
      context.getHandler(),
      context.getClass()
    ]) ?? false;
    ...
    const tenantId = request.user?.tenantId ?? request.headers['x-tenant-id'] ?? request.ip ?? 'anonymous';
    ...
    const [incrResult] = await this.redis
      .multi()
      .incr(key)
      .expire(key, 60)
